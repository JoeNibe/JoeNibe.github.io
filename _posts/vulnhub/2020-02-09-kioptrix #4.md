...
On login we are greeted with a restricted shell. This was my first experience with limited shell. This shell essentially limits the commends user can use.
...
root      4076  0.0  4.6 127140 16452 ?        Sl   01:32   0:07 /usr/sbin/mysqld --basedir=/usr --datadir=/var/lib/mysql --user=root --pid-file=/var/run/mysqld/mysqld.pid --skip-external-locking --port=3306 --socket=/var/run/mysqld/my
...
So essentially we can execute command as `root` using `sys_exec`. But `sys_exec` doesn't return and output.
