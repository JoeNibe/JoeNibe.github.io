...
And we have successfully bypassed the login page and we are presented with some `ssh` credentials. However there is no open `ssh` ports on the system.
...
This is probably because of a entry in `.bashrc` file. We can get a shell by appending `/bin/bash` at the end of the `ssh` command. This ensures that our command is executed before we are kicked out. Rename the `.bashrc` file after getting the shell and then login again.
