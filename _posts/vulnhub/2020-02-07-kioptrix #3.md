...
## Enumeration
As usual add `kioptrix3.com` to `hosts` file and start off with `Nmap`
...
The following details was found

1. 2 users named `loneferret` and `dreg`
```bash
www-data@Kioptrix3:/home$ ls
ls
dreg  loneferret  www
```
2. A file named `gconfig.php` that contains `db` credentials
```bash
...
## User Shell Method 1 (SQL Injection)
...
We can crack the hashes using online rainbow cracker 


## Root Shell
sudo ht. Edit sudoers file, add key to ssh files, change passwd file.

1. Open sudoers file
2. Add `/bin/sh` to the file
